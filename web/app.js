...
function selectMidiPorts() { if (!midi.access) return; const input = populate(midiElements.input, [...midi.access.inputs.values()], midi.input); const output = populate(midiElements.output, [...midi.access.outputs.values()], midi.output); detachMidiInput(); midi.access.inputs.forEach(monitorMidiInput); midi.input = input || null; midi.output = output || null; if (!midi.input && !midi.output) { setMidiStatus("No MIDI ports found"); return; } const inputState = midi.input ? `${midiPortName(midi.input)} (${midi.input.connection || "unknown"})` : "none"; const outputState = midi.output ? midiPortName(midi.output) : "none"; setMidiStatus(`Voice: ${outputState} | Monitor input: ${inputState}`); }
